...
def update_W(W, x, learning_rate):
    ...
    for i in range(d):
        sgn[i][0] = np.sign(W[i].dot(x))
    
    grad = learning_rate*(np.linalg.inv(W.T) - np.outer(sgn,x) )
...
def load_data():
    mix = np.loadtxt('ps3/src/ica/mix.dat')
    return mix

def save_sound(audio, name):
    scipy.io.wavfile.write('ps3/src/ica/{}.wav'.format(name), Fs, audio)
